...
namespace maschinendeck {
  ...
  #include "EEPROM.h"
  inline void printEEPROM(const String &opts) {
    Serial.print("offset \t");
  ...
  struct Command {
    String command;
    void(*callback)(const String &param);
    String description;

    Command(const String &command,  void(*callback)(const String &param), const String &description) : command(command), callback(callback), description(description) {}
  };
  ...
  class SerialTerminal {
    ...
    public:
      ...
      void add(const String &command,  void(*callback)(const String &param), const String &description = "") {
        if (this->size_ >= 64)
          return;
